...
#
# Usage :
#   bash benchmark-llm.sh                       # compare les 6 modèles Ollama
#   RUNS=10 bash benchmark-llm.sh               # 10 runs mesurés par modèle
#   OLLAMA_MODELS="llama3.1:8b llama3.2:3b" bash benchmark-llm.sh
#   OLLAMA_MODEL=llama3.2:3b bash benchmark-llm.sh   # un seul modèle
#   GROQ_API_KEY=gsk_xxx PROVIDERS="ollama groq" bash benchmark-llm.sh
#
...
# ---------------------------------------------------------------------------
RUNS="${RUNS:-5}"                 # nombre de runs MESURÉS par modèle / fournisseur
...
OLLAMA_MODEL="llama3.1:8b"        # écrasé à chaque modèle dans la boucle
...
# Vérifie qu'un modèle est déjà pullé dans Ollama (nom exact).
ollama_has_model() {
    curl -s -m 3 "${OLLAMA_HOST%/}/api/tags" \
...
echo " Modèles Ollama : ${OLLAMA_MODELS}"
echo "==================================================================="
            ...
            if curl -s -m 3 "${OLLAMA_HOST%/}/api/tags" >/dev/null 2>&1; then
                for model in $OLLAMA_MODELS; do
                    if ollama_has_model "$model"; then
                        OLLAMA_MODEL="$model"
                        bench_provider "ollama" call_ollama "$model"
                    else
                        echo ""
                        echo "⏭  modèle ignoré : ${model} (non pullé — \`ollama pull ${model}\`)"
                        MD_ROWS+=("| ollama | ${model} | — | — | — | — | skip |")
                    fi
                done
            else
                echo ""
