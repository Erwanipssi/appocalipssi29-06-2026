#!/usr/bin/env bash
# ============================================================================
# start-linux.sh — Lancement complet du kit EduTutor IA (Linux)
# ----------------------------------------------------------------------------
...
#
# Lançable depuis n'importe ou : le script se replace a la racine du projet.
#
# Usage :
#   bash scripts/start-linux.sh                 # lancement complet
#   bash scripts/start-linux.sh --fast          # relance sans rebuild
#   bash scripts/start-linux.sh --yes           # non-interactif (pull modele auto)
#   bash scripts/start-linux.sh --no-seed       # sans donnees de demo
#   bash scripts/start-linux.sh --logs          # suit les logs apres demarrage
...
if ! command -v docker >/dev/null 2>&1; then
  ...
  echo "  Installez Docker : https://docs.docker.com/engine/install/" >&2
  exit 1
fi
if ! docker info >/dev/null 2>&1; then
  echo "ERREUR : le demon Docker ne repond pas." >&2
  echo "  Demarrez-le : sudo systemctl start docker  (puis relancez ce script)." >&2
  exit 1
fi
if ! docker compose version >/dev/null 2>&1; then
  echo "ERREUR : 'docker compose' (v2) introuvable. Mettez Docker a jour." >&2
  exit 1
fi
